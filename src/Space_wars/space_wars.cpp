...
void drawBackground()
{
  ...
  sw_x=sw_x+changeX;
  if(sw_x==imageW-screenW-1 || sw_x<0)
  changeX=changeX*-1;

   sw_y=sw_y+changeY;
  if(sw_y==imageH-screenH-1 || sw_y<0)
  changeY=changeY*-1;
...
void space_warsLoop() {
 ...
 checkColision();
  ...
  if(digitalRead(up)==LOW && py>0)
  py=py-3;
  if(digitalRead(down)==LOW && py<140-28)
  py=py+3;

  if(digitalRead(left)==LOW && px>0)
  px=px-3;
  if(digitalRead(right)==LOW && px<290)
  px=px+3;

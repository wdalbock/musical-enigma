...
int updateBall() {
    ...
    if (ball.y <= paddles[0].y + paddles[0].height && ball.x >= paddles[0].x && ball.x <= paddles[0].x + paddles[0].width) {
        ...
        ball.dx = ball.dx + isMoving[0] * ((paddles[0].speed * paddleDirection[0])/10); // The speed of the ball is influenced by the current speed/direction of the paddle.
    ...
    if (ball.y >= paddles[1].y - 5 && ball.x >= paddles[1].x && ball.x <= paddles[1].x + paddles[1].width) {
        ball.dy = -ball.dy; 
        ball.dx = ball.dx + isMoving[1] * ((paddles[1].speed * paddleDirection[1])/10);
        collisions++;
        playCollisionSound();
    }
    ...
    if (ball.x <= paddles[2].x + paddles[2].width && ball.y >= paddles[2].y && ball.y <= paddles[2].y + paddles[2].height) {
        ball.dx = -ball.dx; 
        ball.dy = ball.dy + isMoving[2] * ((paddles[2].speed * paddleDirection[2])/10);
        collisions++;
        playCollisionSound();
    }
    ...
    if (ball.x >= paddles[3].x - 5 && ball.y >= paddles[3].y && ball.y <= paddles[3].y + paddles[3].height) {
        ball.dx = -ball.dx;
        ball.dy = ball.dy + isMoving[3] * ((paddles[3].speed * paddleDirection[3])/10);
        collisions++;
        playCollisionSound();
    }
